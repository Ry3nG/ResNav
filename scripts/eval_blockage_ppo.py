...
def make_env(seed: int, scen_cfg: BlockageScenarioConfig, render: bool):
    def _thunk():
        ...
        env = BlockageRLWrapper(env, scenario_cfg=scen_cfg, reward_cfg=RewardConfig(), seed=seed)
        return env
...
def main():
        ...
        while not done:
            ...
            if args.render:
                try:
                    ...
                    total_wp = base_env.path_waypoints.shape[0] if base_env.path_waypoints is not None else 1
    ...
    print(f"Collision:{results['collision']}/{total} ({results['collision']/total*100:.1f}%)")
    print(f"Timeout:  {results['timeout']}/{total} ({results['timeout']/total*100:.1f}%)")
